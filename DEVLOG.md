...
## 2026-07-23 作品欣赏扩容至 197 幅画作

### 扩容内容
- 超现实主义从 5 幅扩展到 40 幅（达利、马格利特、夏加尔）
- 新增 16 个流派共 157 幅画作：
  - 文艺复兴（20）、巴洛克（15）、新古典主义（8）、浪漫主义（15）
  - 现实主义（8）、印象派（18）、后印象派（15）、北方文艺复兴（8）
  - 表现主义（8）、立体主义（6）、波普艺术（5）、现代艺术（10）
  - 野兽派（5）、象征主义（6）、战争历史（5）、亚洲艺术（5）
- 总计 197 幅画作，涵盖从达芬奇到沃霍尔的各大名家
...
### 图片来源
- 使用 WikiArt 图片 CDN 直链
- URL 统一添加年份后缀以提高正确率
- 若国内访问受限可后续下载到本地服务器

### 技术改进
- 箭头改为半透明毛玻璃效果（`rgba(0,0,0,0.06)` + `backdrop-filter: blur(6px)`）
- 计数器改为半透明背景
- 切换视频时自动停止前一个 iframe 播放
- 首页右侧替换重复和有黑背景的 GIF
- 视频欣赏移除多余版权声明，只保留页脚统一版权
---

## 2026-07-24 首页GIF替换 + 作品欣赏去重与优化 + 视频箭头BUG修复

### 首页右侧GIF替换
- 右侧第5个派大星扭屁股 GIF（`clapcheeks.gif`）替换为海绵宝宝 GIF（picgifs 源）

### 作品欣赏页面去重与加载优化
- 扫描 `public/uploads/images/gallery/` 下 97 张图片，通过 MD5 内容哈希去重
- 清除 21 张完全重复的图片（同文件内容不同文件名），保留 76 张唯一作品
- `gallery.astro` 改为运行时读取目录，动态包含所有图片
- 5 张知名画作保留完整元数据（名称、作者、年份、馆藏地）；其余自动编号
- 加载优化：非首张图片使用 `loading="lazy"` 懒加载
- JS 预加载当前/上一张/下一张图片（`preloadAdjacent`）
- 图片加载失败时显示 "❌ 加载失败" 占位提示

### 视频欣赏箭头BUG修复
- 根因：B站 iframe 捕获鼠标事件，`document mousemove` 不触发
- 4 秒自动隐藏后箭头永远不会再出现
- 修复：移除视频页的自动隐藏逻辑，箭头/计数器/信息始终可见

---

## 2026-07-24 图片转 WebP 批量压缩 + 视频箭头自动隐藏修复

### WebP 批量转换（大幅减小体积）
- 76 张画廊图片全部转 WebP（quality=80）
- 对 WebP 反而更大的 20 张保留原 JPG
- 结果：**64.2 MB → 23.1 MB，节省 64% 空间**
- 更新 `gallery.astro` 中 5 张知名画作的文件名映射
